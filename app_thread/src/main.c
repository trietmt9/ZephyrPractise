...
k_thread_runtime_stats_t rt_led_stats_thread;
k_thread_runtime_stats_t rt_log_stats_thread;
...
void led_blink_thread(void *arg1, void *arg2, void* arg3)
{
    ...
    while(1)
    {
        ...
        k_thread_runtime_stats_get(k_current_get(), &rt_led_stats_thread);
        LOG_INF("Cycle LOG: %llu", rt_led_stats_thread.execution_cycles);
        k_msleep(20);
    }
...
void log_print_thread(void *arg1, void *arg2, void *arg3)
{
    ...
    while(1)
    {
        k_thread_runtime_stats_get(k_current_get(), &rt_log_stats_thread);
        LOG_INF("Cycle LED: %llu", rt_log_stats_thread.execution_cycles);
        k_msleep(20);
    }
}
...
int main(void)
{
    ...
    * Create LOG thread 
    */
    k_thread_create(&log_thread_data, 
                    log_thread_stack,
